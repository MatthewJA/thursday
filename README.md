...
6. Use the `fit` method to train the model. Both have `train_x` and `train_y` as inputs. (Note: For the `HOGnet` `fit` method, the the labels must first be converted to a confusion matrix using the `keras.utils.to_categorical` function. This is to satisfy the shape requirements for the binary cross-entropy loss function in keras.)
